...
async function connectToMongoose(delay = 250) { // Default delay value
  ...
  const connect = async () => {
    ...
    try {
      const mongooseConnection = await mongoose.createConnection(process.env.DB_CONNECTION_STRING, { dbName: process.env.DB_NAME }).asPromise();
      ...
      if (attempts < maxRetries) {
        ...
        setTimeout(connect(nextDelay), delay); // Retry connection
      } else {
